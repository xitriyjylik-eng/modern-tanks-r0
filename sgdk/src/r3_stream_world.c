...
static const u8 * const r3TileChunks[40] =
{
    ...
    r3_target_world_tiles_30,
    r3_target_world_tiles_31,
    r3_target_world_tiles_32,
    r3_target_world_tiles_33,
    r3_target_world_tiles_34,
    r3_target_world_tiles_35,
    r3_target_world_tiles_36,
    r3_target_world_tiles_37,
    r3_target_world_tiles_38,
    r3_target_world_tiles_39
};
...
static s32 smoothCameraAxis(s32 current, s32 target)
{
    ...
    if ((delta > -R3_CAM_SNAP_Q8) && (delta < R3_CAM_SNAP_Q8)) return target;
...
static void loadWorldTile(s16 worldX, s16 worldY, TransferMethod tm)
{
    ...
    const u16 chunk = wy / 3;
    const u16 localRow = wy % 3;
    ...
    VDP_setTileMapXY(BG_B, TILE_ATTR_FULL(palette, FALSE, FALSE, FALSE, patternIndex), planeX, planeY);
}

static void loadColumn(s16 worldX, s16 baseY, TransferMethod tm)
{
    s16 y;
    for (y = 0; y < R3_RING_H; y++) loadWorldTile(worldX, baseY + y, tm);
}

static void loadRow(s16 baseX, s16 worldY, TransferMethod tm)
{
    s16 x;
    for (x = 0; x < R3_RING_W; x++) loadWorldTile(baseX + x, worldY, tm);
}
...
static void drawHud(void)
{
    ...
    VDP_fillTileMapRect(BG_A, TILE_ATTR_FULL(PAL0, TRUE, FALSE, FALSE, R3_HUD_BG_TILE), 28, 0, 12, 24);
    VDP_fillTileMapRect(BG_A, TILE_ATTR_FULL(PAL0, TRUE, FALSE, FALSE, R3_HUD_BG_TILE), 0, 24, 40, 4);
    ...
    VDP_fillTileMapRect(BG_A, TILE_ATTR_FULL(PAL2, TRUE, FALSE, FALSE, R3_HUD_SOLID_TILE), 32, 4, 6, 1);
    VDP_fillTileMapRect(BG_A, TILE_ATTR_FULL(PAL1, TRUE, FALSE, FALSE, R3_HUD_SOLID_TILE), 32, 7, 5, 1);
